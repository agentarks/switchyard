...
- bounded headless Codex launch via `codex exec --json` with a default `workspace-write` sandbox
...
- status cleanup-readiness now also refuses `ready:merged` when a preserved worktree still has uncommitted non-Switchyard entries
- status cleanup-readiness now also fails closed when Switchyard cannot inspect a preserved worktree for those entries
...
- `sy sling [args...]`
  ...
  - launches one bounded `codex exec --json` task from that worktree, defaulting to `--sandbox workspace-write` unless the operator already chose another sandbox or automation mode
...
- `sy status [session]`
  ...
  - refuses merged-safe cleanup readiness when a preserved worktree still has uncommitted non-Switchyard entries, even if the branch is already merged into the stored base branch
  - fails cleanup readiness closed when Switchyard cannot inspect a preserved worktree for uncommitted entries, instead of silently treating it as merged-safe
  ...
  - includes one conservative `REVIEW` assessment per inactive session in that table so operators can see `ready`, `needs-review`, `blocked`, `risky`, or `-` without changing the existing `NEXT` vocabulary
...
- `sy stop <session>`
  ...
  - refuses plain merged-cleanup when the preserved worktree still has uncommitted non-Switchyard entries, and tells the operator to resolve or explicitly abandon them
  - refuses plain merged-cleanup when Switchyard cannot inspect the preserved worktree for those entries, and tells the operator to resolve that inspection failure or explicitly abandon the worktree
