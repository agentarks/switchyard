...
3. Reintegrate from the canonical branch in the main repository.
   ...
   - The command checks that the session is no longer active, verifies the preserved branch still exists, verifies that the preserved worktree path still resolves to the expected git worktree root, requires both the preserved agent worktree and the repo-root worktree to be clean, switches to the configured canonical branch, and then runs the explicit git merge.
