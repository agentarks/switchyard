...
test("slingCommand creates a worktree and persists a started session", async () => {
  ...
  let spawnedLogPath: string | undefined;
  const writes: string[] = [];
  ...
  try {
    await slingCommand({
      ...
      spawnRuntime: async ({ runtimeArgs, logPath, onSpawned }) => {
        assert.deepEqual(runtimeArgs, ["exec", "--json", "--sandbox", "workspace-write", "--model", "gpt-5", task]);
  ...
  assert.equal(spawnedLogPath, join(repoDir, ".switchyard", "logs", `agent-one-${sessions[0]?.id}.log`));
  ...
  assert.equal(spawnedEvent?.payload.runtimeCommand, "codex exec --json --sandbox workspace-write --model gpt-5");
  ...
  assert.equal(completedEvent?.payload.runtimeCommand, "codex exec --json --sandbox workspace-write --model gpt-5");
  ...
  assert.match(writes.join(""), /Runtime: codex exec --json --sandbox workspace-write --model gpt-5/);
...
test("slingCommand preserves an explicit sandbox override in the launch command", async () => {
  const repoDir = await createInitializedRepo();

  try {
    await slingCommand({
      agentName: "Agent Explicit Sandbox",
      task: "Confirm the launch sandbox is preserved.",
      runtimeArgs: ["--sandbox", "read-only"],
      startDir: repoDir,
      spawnRuntime: async ({ runtimeArgs, onSpawned }) => {
        assert.deepEqual(runtimeArgs, ["exec", "--json", "--sandbox", "read-only", "Confirm the launch sandbox is preserved."]);
        const runtime = {
          pid: 6262,
          command: {
            command: "codex",
...
test("statusCommand promotes a started session to running after the first successful liveness check", async () => {
  ...
  try {
    ...
    assert.match(
      output,
      /running\t[0-9a-f-]+\tagent-two\tagents\/agent-two\t\.switchyard\/worktrees\/agent-two\t2026-03-09T09:10:00.000Z\t0\tstop-then:merged\tValidate that the runtime transitions to running\.\tactive\twait\t2026-03-09T09:10:00.000Z runtime\.ready signal=pid_alive, runtimePid=31337/
    );
  } finally {
    await removeTempDir(repoDir);
  }
